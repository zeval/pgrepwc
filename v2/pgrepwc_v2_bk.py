...
def main(argv):
    ...
    if parallelization:
        ...
        for process in range(numberOfProcesses):
            ...
            while fileIndex < len(allFiles):

                fileSize = os.path.getsize(allFiles[fileIndex])
                numberOfProcessesForFile = ceil(fileSize/bytesPerProcess)

                

                if (allFiles[fileIndex] in processTable):
                    previousProcess = processTable[allFiles[fileIndex]][-1][1]

                    if previousProcess.getEnd() == fileSize:

                        fileIndex += 1
                        continue

                if allFiles[fileIndex] not in processTable:
                    processTable[allFiles[fileIndex]] = []
                    if fileSize <= bytesPerProcess:
                        dataToHandle = Load(allFiles[fileIndex], 0, fileSize)

                    else:
                        dataToHandle = Load(allFiles[fileIndex], 0, bytesPerProcess)
                else:

                    previousProcess = processTable[allFiles[fileIndex]][-1][1]

                    # If the remaining bytes to read are less than the average:
                    if previousProcess.getEnd() + 1 + bytesPerProcess > fileSize:

                        # Load(file, where_previous_process_ended + 1, rest_of_file)
                        dataToHandle = Load(allFiles[fileIndex], previousProcess.getEnd() + 1, fileSize - previousProcess.getEnd() - 1)
                    else: 
                        # Load(file, where_previous_process_ended + 1, average_bytes_per_process)
                        dataToHandle = Load(allFiles[fileIndex], previousProcess.getEnd() + 1, bytesPerProcess)


                processTable[allFiles[fileIndex]].append((Process(target=matchFinder, args=(dataToHandle, opts, args[0], totalWC, totalLC, mutex)), dataToHandle))
